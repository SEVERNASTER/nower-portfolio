...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "userImageChanged",
      handleUserImageChange as EventListener,
    );
    return () => {
  ...
  return (
    <aside
      className={`fixed inset-y-0 left-0 z-50 w-72 transform flex-col bg-white dark:bg-[#17262C] border-r border-slate-200 dark:border-slate-800/60 transition-transform duration-300 lg:static lg:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"} flex`}
    >
      ...
      <div className="border-t border-slate-200 dark:border-slate-800/60 p-4">
        <div className="mb-4 flex items-center justify-between px-2">
          ...
          <button
            onClick={toggleTheme}
            className="relative inline-flex h-6 w-11 items-center rounded-full bg-slate-200 dark:bg-slate-700 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-[#162033]"
          >
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${isDark ? "translate-x-6" : "translate-x-1"}`}
            />
            {isDark ? (
              <Moon className="absolute left-1.5 h-3 w-3 text-slate-400" />
            ) : (
              <Sun className="absolute right-1.5 h-3 w-3 text-amber-500" />
            )}
          </button>
        </div>
