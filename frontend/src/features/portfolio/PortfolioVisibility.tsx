...
const templates: Array<{
...
}> = [
  {
    ...
    name: "Clásica",
    description:
      "Orden profesional tradicional: perfil, proyectos, habilidades, experiencia y logros.",
  },
  {
    key: "modern",
    name: "Moderna",
    description:
      "Diseño con columna lateral para datos personales y contenido principal destacado.",
  },
  {
    key: "creative",
    name: "Creativa",
    description:
      "Presentación más visual, ideal para portafolios con imagen y proyectos destacados.",
  },
];
...
export const PortfolioVisibility: React.FC = () => {
  ...
  const [selectedTemplate, setSelectedTemplate] =
    useState<PortfolioTemplateKey>("classic");
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      setError(
        e instanceof Error ? e.message : "Error enviando portafolio a revisión",
      );
    } finally {
  ...
  const executeUnpublish = async () => {
    ...
    try {
      ...
      setError(
        e instanceof Error ? e.message : "Error despublicando portafolio",
      );
    } finally {
      setActionLoading(false);
    }
    ...
    <div className="w-full max-w-6xl mx-auto space-y-8 animate-fade-in">
          ...
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                Selecciona una plantilla, previsualiza tu portafolio y publícalo
          ...
          <div className="mt-5 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-medium text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-950/20 dark:text-emerald-300">
            <div className="flex items-start gap-3">
              <p>
                ...
                Tu portafolio ya está visible en el panel de inicio junto a los
                portafolios públicos de los demás usuarios.
              </p>
            </div>
          </div>
        )}
        {status?.status === "published" &&
          ...
          status.content_dirty && (
            <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm font-medium text-amber-700 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-300">
              Tienes cambios nuevos en tu portafolio. Vuelve a publicarlo para
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm text-rose-700 dark:text-rose-400">
                  El administrador ha revisado tu portafolio y ha determinado
                  que es necesario realizar algunos ajustes antes de publicarlo.
                </p>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800/60 dark:bg-[#17262C]">
        ...
        <div className="scale-[0.92] origin-top">
          {previewUser &&
            renderPortfolioTemplate(selectedTemplate, previewUser)}
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800/60 dark:bg-[#17262C]">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-300 lg:max-w-2xl">
            Antes de ser publicado y visible para visitantes y reclutadores, tu
            portafolio será revisado por los administradores para su aprobación
            o rechazo.
          </p>
